...
extension NSError {
    public convenience init(httpCode: Int) {
        let message = CODE_ERROR_MESSAGE[httpCode] ?? "Unknown HTTP Status Code." + " (\(httpCode))"
        self.init(domain: "Task Error", code: httpCode, userInfo: [NSLocalizedDescriptionKey: message])
    }

    public convenience init(errorMessage msg: String?) {
        let message = msg ?? "未知错误"
        self.init(domain: "Task Error", code: 600, userInfo: [NSLocalizedDescriptionKey: message])
    }
}
...
public let CODE_ERROR_MESSAGE: [AnyHashable: String] = [
    ...
    -1001 : "请求超时",
    -1003 : "无法连接到服务器，请检查网络配置",
    -1004 : "无法连接到服务器，请检查网络设置",
    -1009 : "无网络连接，请确认设备已连接到互联网",
    -1022 : "App Transport Security policy requires the use of a secure connection.",
